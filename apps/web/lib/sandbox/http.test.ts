...
describe("sandboxErrorResponse", () => {
  it("maps GitHub authentication and rate-limit failures", async () => {
    const unauthorized = sandboxErrorResponse(new GitHubApiError("Bad credentials", 401));
    expect(unauthorized.status).toBe(401);
    await expect(unauthorized.json()).resolves.toMatchObject({ code: "authentication_required" });

    const limited = sandboxErrorResponse(new GitHubApiError("API rate limit exceeded", 429));
  ...
  it("maps GitHub not-found and forbidden responses", async () => {
    const missing = sandboxErrorResponse(new GitHubApiError("Not Found", 404));
    expect(missing.status).toBe(404);
    await expect(missing.json()).resolves.toMatchObject({ code: "github_not_found" });

    const forbidden = sandboxErrorResponse(new GitHubApiError("Resource not accessible", 403));
