...
2. Select a repository, then clone it into the persistent sandbox. A first clone opens a fresh terminal already in that directory; an existing clone stays in the current terminal and changes directory there. The active repository is shown above the terminal for the current browser session.
